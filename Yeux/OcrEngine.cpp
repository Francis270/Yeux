...
bool OcrEngine::loadTesseract(std::string const& lang)
{
...
void OcrEngine::screenshot(int x, int y, int width, int height, std::string const& path)
{
...
void OcrEngine::clean()
{
	pixDestroy(&m_pixels);
}

std::string const OcrEngine::getText(Region const& region, bool const debug) const
{
...
RGB const OcrEngine::getRGB(Region const& region, bool const debug) const
{
	RGB rgb = {0, 0, 0};
	l_uint32 val;
...
std::string const OcrEngine::getColor(Region const& region, bool const debug) const
{
	std::string color = "";
...
Pix* OcrEngine::hbitmapToPixs(DWORD dwBmpSize, BITMAPFILEHEADER bmfHeader, BITMAPINFOHEADER bi, char* lpbitmap)
{
...
wchar_t* OcrEngine::stringToWideChar(std::string const& string)
{
	int len;
...
PBITMAPINFO OcrEngine::createBitmapInfoStruct(HBITMAP hBmp)
{
	BITMAP bmp;
	PBITMAPINFO pbmi;
	WORD    cClrBits;
...
void OcrEngine::createBMPFile(LPTSTR pszFile, HBITMAP hBMP)
{
	HANDLE hf;
...
Pix* OcrEngine::cropPix(Pix* pixs, int const x, int const y, int const x2, int const y2) const
{
	if (x2 - x <= 0 || y2 - y <= 0) {
		return pixs;
	}
...
Pix* OcrEngine::processPix(Pix* pixd, float scaleFactor, bool s) const
{
...
std::pair<std::string, int> const OcrEngine::tesseractCompute(Pix* pixs, std::string const& whiteList, bool debug) const
{
	m_tapi->SetImage(pixs);
	
	std::pair<std::string, int> ret;
	char* text = m_tapi->GetUTF8Text();
	std::string tmp(text);
